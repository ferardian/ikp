...
<head>
    ...
    <title>{{ $settings->name ?? 'Insiden Keselamatan Pasien' }}</title>
    ...
    <div class="max-w-7xl mx-auto relative py-7">
        <div class="flex flex-col md:flex-row-reverse xl:flex-row gap-6 xl:h-[calc(100svh-16rem)]">
            ...
            <div class="flex flex-col items-start justify-center gap-3 xl:gap-6 flex-1 max-w-xl px-5 xl:pl-10 2xl:px-0">
                <p class="text-base xl:text-[22px] font-semibold text-[#00BFA5]">{{ $settings->faskes }}</p>
                <h1 class="text-4xl xl:text-5xl font-bold text-black">{{ $settings->name }}</h1>
                <p class="text-base text-[#6C87AE]">{{ $settings->about }}</p>

                <div class="mt-7 xl:mt-0">
                    @auth
                        <a href="{{ route('filament.app.pages.dashboard') }}" class="text-lg leading-none text-white rounded-full px-5 py-3 flex items-center justify-center gap-3 bg-gradient-to-br from-[#3A8EF6] to-[#6F3AFA] shadow-lg shadow-gray-400/50 hover:-translate-y-1 cursor-pointer transition-all ease-in-out duration-300">
                            <x-icons.health-recognition class="w-6 h-6 text-white" />
                            <span class="mb-0.5">Manage Your Risk</span>
                        </a>
                    @else
                        <a href="{{ route('filament.app.auth.login') }}" class="text-lg leading-none text-white rounded-full px-5 py-3 flex items-center justify-center gap-3 bg-gradient-to-br from-[#3A8EF6] to-[#6F3AFA] shadow-lg shadow-gray-400/50 hover:-translate-y-1 cursor-pointer transition-all ease-in-out duration-300">
                            <x-icons.health-recognition class="w-6 h-6 text-white" />
                            <span class="mb-0.5">Manage Your Risk</span>
                        </a>
                    @endauth
